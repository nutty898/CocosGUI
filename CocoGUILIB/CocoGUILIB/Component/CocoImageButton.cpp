...
namespace cs
{
    ...
    bool CocoImageButton::init()
    {
        if (CocoButton::init())
        {
            if (m_bScale9Enable)
            {
                m_pImageNormal = new UIScale9Sprite();
                m_pImageClicked = new UIScale9Sprite();
                m_pImageDisable = new UIScale9Sprite();
            }
            else
            {
                m_pImageNormal = new UISprite();
                m_pImageClicked = new UISprite();
                m_pImageDisable = new UISprite();
            }
            m_pImageNormal->init();
            m_pImageClicked->init();
            m_pImageDisable->init();
            addUIElement(m_pImageNormal);
            addUIElement(m_pImageClicked);
            addUIElement(m_pImageDisable);
    ...
    void CocoImageButton::setImageScale9Enable(bool able)
    {
        ...
        removeUIElement(m_pImageNormal, true);
        removeUIElement(m_pImageClicked, true);
        removeUIElement(m_pImageDisable, true);
        m_pImageNormal = NULL;
        m_pImageClicked = NULL;
        m_pImageDisable = NULL;
    ...
    CRenderNode* CocoImageButton::getImageValidNode()
    {
        ...
        {
            case 0:
                res = m_pImageNormal->getCRenderNode();
                break;
                
            case 1:
                res = m_pImageClicked->getCRenderNode();
                break;
                
            case 2:
                res = m_pImageDisable->getCRenderNode();
                break;
